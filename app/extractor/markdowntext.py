...
# """
# Convert document to Markdown text with page chunks and output as JSON.
# Intelligently removes headers and footers by detecting repetitive patterns.
...
#     # Only normalize page numbers - everything else (versions, copyright, etc.) stays the same
#     # This will detect patterns like "Version 2014 Page 2 of 12 May 31, 2014" across all pages
...
# def identify_header_footer_patterns(page_analyses: List[Dict], min_frequency: float = 0.7) -> Tuple[Set[str], Set[str]]:
#     """
...
#     # Collect all lines and their positions
...
#             # Top 30% = header, bottom 30% = footer
#             if avg_position <= 0.3:
#                 header_patterns.add(pattern)
#                 print(f"  Header pattern: '{pattern}' (appears on {len(info['pages'])} pages)")
#                 print(f"    Examples: {info['examples'][:2]}")
#             elif avg_position >= 0.7:
#                 footer_patterns.add(pattern)
#                 print(f"  Footer pattern: '{pattern}' (appears on {len(info['pages'])} pages)")
#                 print(f"    Examples: {info['examples'][:2]}")
...
#     md_data = pymupdf4llm.to_markdown(input_pdf_path, page_chunks=True,write_images=False)
...
#     header_patterns, footer_patterns = identify_header_footer_patterns(page_analyses, min_frequency=0.7)
...
#     # Step 4: Create final line-by-line structure (same format as current file02.json)
...
#     # Create final JSON structure (same format as current file02.json)
#     final_output = {
#         "metadata": metadata,
#         "total_lines": len(lines_data),
#         "total_pages": len(md_data),
