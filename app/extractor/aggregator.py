...
def aggregate_md_to_spans(pdf_name: str):
    """
    Main function to aggregate MD JSON lines with corresponding spans for a given PDF
    Args:
        pdf_name (str): Name of the PDF file (e.g., 'file06.pdf')
    """
    ...
    # Define file paths - Updated to use JSON instead of MD
    md_json_file = f"../../data/md_files/{base_filename}.json"  # Changed from .md to .json
    spans_json_file = f"../../data/spans_output/spans_{pdf_name}.json"
    output_file = f"../../data/aggregator_output/aggregated_{pdf_name}.json"
    
    start_time = time.time()
    
    print(f"Loading spans data from {spans_json_file}...")
    load_start = time.time()
    ...
    print(f"Loading markdown JSON from {md_json_file}...")
    md_start = time.time()
    metadata, md_lines = load_md_json_file(md_json_file)  # Updated function call
    md_end = time.time()
    ...
    for md_line_data in md_lines:
        ...
        # Find best matching span within the same page
    ...
    # Calculate page-wise statistics
    page_stats = {}
